...
class SearchMixin(object):
    def fbUserSearch(self, query):
        query = self.SendRequest(
            'fbsearch/topsearch/?context=blended&query=' +
    ...
    def searchUsers(self, query):
        query = self.SendRequest(
            'users/search/?ig_sig_key_version=' +
            str(config.SIG_KEY_VERSION) +
            '&is_typeahead=true&query=' + str(query) +
    ...
    def searchTags(self, query):
        query = self.SendRequest(
            'tags/search/?is_typeahead=true&q=' +
            str(query) + '&rank_token=' + str(self.rank_token)
        )
        return query
    ...
    def searchLocation(self, query, lat=None, lng=None):
        locationFeed = self.SendRequest(
            'fbsearch/places/?rank_token=' + str(self.rank_token) +
            '&query=' + str(query) + '&lat=' + str(lat) + '&lng=' + str(lng)
        )
        return locationFeed
